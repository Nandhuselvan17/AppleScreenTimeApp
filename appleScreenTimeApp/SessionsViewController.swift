...
//        loadSchedules()
        ...
        timer = Timer.scheduledTimer(timeInterval: 30.0, target: self, selector: #selector(checkTime), userInfo: nil, repeats: true)
    }
    ...
    @objc func checkTime() {
        ...
        for schedule in schedules {
            let times = schedule.time.split(separator: "-").map { $0.trimmingCharacters(in: .whitespaces) }
            if times.count == 2, let startTime = times.first, let endTime = times.last {
                if startTime == currentTime || endTime == currentTime {
                    let rules = DataStore.shared.selectedCategories
//                    cell.textLabel?.text = "\(rules.joined(separator: ", ")) Apps Blocked"
                    showAlert(message: "The time is \(currentTime). Time reached! And \(rules.joined(separator: ", ")) Apps Blocked ")
                }
            }
        }
    }
    ...
    func showAlert(message: String) {
        let alertController = UIAlertController(title: "Time Alert", message: message, preferredStyle: .alert)
